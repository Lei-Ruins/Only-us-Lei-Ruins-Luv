...
export function usePeer() {
  ...
  useEffect(() => {
    ...
    newPeer.on('call', (call) => {
      ...
      console.log('Answering incoming call synchronously with cached tracks:', stream.getTracks().length);
      
      call.answer(stream);
      call.on('stream', (rStream) => {
        console.log('Successfully received remote stream during call lifecycle:', rStream.id);
      ...
      call.on('close', () => {
        console.log('Incoming media call stream closed');
  ...
  const startVideoCall = useCallback(async (id: string) => {
    ...
    try {
      ...
      call.on('close', () => {
        console.log('Outbound media call stream closed');
        setRemoteStream(null);
      });
      call.on('error', (err) => {
        console.error('Outbound media call error:', err);
      });
    } catch (err) {
